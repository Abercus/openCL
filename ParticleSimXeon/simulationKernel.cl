...
__kernel void bin_init_gpu(volatile __global int * binSizes, int numberOfBins) {
...
// Bins particles
__kernel void bin_gpu(__global particle_t * particles, int n, __global particle_t * bins, volatile __global int* binSizes, int numberOfBins) {
        int tid = get_global_id(0);
...
//	print("%i %i \n", r, t);
        ...
        // atomically add .
        int h = atom_add(&binSizes[binNr], 1);
...
__kernel void compute_forces_gpu(__global particle_t * particles,
        int n, __global particle_t * bins, volatile __global int* binSizes, int numberOfBins)
{
